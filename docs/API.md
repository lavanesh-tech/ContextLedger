...
| Revocations | `POST …/facts/{fact_id}/revoke` (`reason`, optional `version_id`; needs `facts:write` and `decisions:read`; 409 if already revoked); `GET …/facts/{fact_id}/impact`; `GET …/fact-versions/{id}/revocation`; `GET …/revocations` |
